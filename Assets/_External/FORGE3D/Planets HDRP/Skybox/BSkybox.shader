	...
	SubShader
	{
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_Stars = packedInput.ase_texcoord1.xy * _Stars_ST.xy + _Stars_ST.zw;
				float4 temp_output_12_0 = saturate( ( ( tex2D( _Stars, uv_Stars ) * _Brightness * _StarsTint ) + _SkyTint ) );
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 uv_Stars = packedInput.ase_texcoord2.xy * _Stars_ST.xy + _Stars_ST.zw;
				float4 temp_output_12_0 = saturate( ( ( tex2D( _Stars, uv_Stars ) * _Brightness * _StarsTint ) + _SkyTint ) );
...
ASEEND*/
//CHKSM=A6B63C4A2871EA9A04E49AEDABF81B7C5473888F